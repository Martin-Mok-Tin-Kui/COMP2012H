...
Cell* eval_listp(Cell* c) {
  if (!check_form(c, 1, 1)) {
    throw runtime_error("operator listp expects one operand.");
  }
  if (listp(car(c))) {
    return make_int(1);
  }
...
Cell* eval_symbolp(Cell* c) {
  if (!check_form(c, 1, 1)) {
    throw runtime_error("operator symbolp expects one operand.");
  }
  if (symbolp(car(c))) {
    return make_int(1);
  }
...
Cell* eval_intp(Cell* c) {
  if (!check_form(c, 1, 1)) {
    throw runtime_error("operator intp expects one operand.");
  }
  if (intp(car(c))) {
    return make_int(1);
  }
...
Cell* eval_nump(Cell* c) {
  if (!check_form(c, 1, 1)) {
    throw runtime_error("operator nump expects one operand.");
  }
  if (intp(car(c)) || doublep(car(c))) {
    return make_int(1);
  }
  else {
    return make_int(0);
...
Env* init_env() {
  ...
  global_f->define("listp", make_primitive(eval_listp));
  global_f->define("symbolp", make_primitive(eval_symbolp));
  global_f->define("intp", make_primitive(eval_intp));
  global_f->define("nump", make_primitive(eval_nump));
  return env;
}
